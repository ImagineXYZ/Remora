...
boolean Adafruit_FONA::incomingCallNumber(char* phonenum)
{
	...
	DEBUG_PRINT(F("Phone Number: ")); DEBUG_PRINTLN(replybuffer);
		...
		uint16_t *readlen)
{
	...
	DEBUG_PRINT(F("AT+CMGR=")); DEBUG_PRINTLN(i);
...
int8_t Adafruit_FONA::GPSstatus(void)
{
	...
	else
	{
		...
		char *p = prog_char_strstr(replybuffer,
				(prog_char*)F("SSTATUS: Location "));
		if (p == 0)
			return -1;
		p += 18;
		readline(); // eat 'OK'
...
boolean Adafruit_FONA::getGPS(double *date, float *lat, float *lon,
		float *speed_kph, float *heading, float *altitude, bool *run, bool *fix,
		uint8_t *fix_mode, float *hdop, float *pdop, float *vdop,
		uint8_t *gps_view, uint8_t *gns_used, float *hpa, float *vpa)
{
	...
	*lat = atof(strtok_single(NULL, ",")); // Gets latitude

	*lon = atof(strtok_single(NULL, ",")); // Gets longitude
	...
	*gns_used = atoi(strtok_single(NULL, ",")); // Gets GNSS used
	...
	strtok_single(NULL, ","); // Gets GLONASS in view

	strtok_single(NULL, ","); // Reserved 3

	strtok_single(NULL, ","); // Gets C/N0 max
...
boolean Adafruit_FONA::enableGPRS(boolean onoff)
{
	...
	if (onoff)
	{
		...
		if (apn)
		{
			...
			DEBUG_PRINT(F("\t---> "));DEBUG_PRINT(F("AT+CSTT=\"")); DEBUG_PRINT(apn);

			if (apnusername)
			{
				DEBUG_PRINT("\",\""); DEBUG_PRINT(apnusername);
			}
			if (apnpassword)
			{
				DEBUG_PRINT("\",\""); DEBUG_PRINT(apnpassword);
			} DEBUG_PRINTLN("\"");
			...
			if (apnusername)
			{
				...
				if (!sendCheckReplyQuoted(F("AT+SAPBR=3,1,\"USER\","),
						apnusername, ok_reply, 10000))
					return false;
			}
			if (apnpassword)
			{
				// Send command AT+SAPBR=3,1,"PWD","<password>" where <password> is the configured APN password.
				if (!sendCheckReplyQuoted(F("AT+SAPBR=3,1,\"PWD\","),
						apnpassword, ok_reply, 10000))
					return false;
			}
		}
...
boolean Adafruit_FONA_3G::enableGPRS(boolean onoff)
{
	...
	if (onoff)
	{
		...
		if (apn)
		{
			...
			if (!sendCheckReplyQuoted(F("AT+CGSOCKCONT=1,\"IP\","), apn,
					ok_reply, 10000))
				return false;
...
boolean Adafruit_FONA::TCPconnect(char *server, uint16_t port)
{
	...
	DEBUG_PRINT(F("AT+CIPSTART=\"TCP\",\"")); DEBUG_PRINT(server); DEBUG_PRINT(F("\",\"")); DEBUG_PRINT(port); DEBUG_PRINTLN(F("\""));
...
boolean Adafruit_FONA::TCPsend(char *packet, uint8_t len)
{
	...
	DEBUG_PRINT(F("AT+CIPSEND=")); DEBUG_PRINTLN(len);
#ifdef ADAFRUIT_FONA_DEBUG
		...
		boolean quoted)
{
	...
	DEBUG_PRINT(F("\t---> ")); DEBUG_PRINT(F("AT+HTTPPARA=\"")); DEBUG_PRINT(parameter); DEBUG_PRINTLN('"');
...
boolean Adafruit_FONA::HTTP_data(uint32_t size, uint32_t maxTime)
{
	...
	DEBUG_PRINT(F("\t---> ")); DEBUG_PRINT(F("AT+HTTPDATA=")); DEBUG_PRINT(size); DEBUG_PRINT(','); DEBUG_PRINTLN(maxTime);
		...
		uint16_t *datalen)
{
	...
	DEBUG_PRINT(F("Status: "));DEBUG_PRINTLN(*status); DEBUG_PRINT(F("Len: "));DEBUG_PRINTLN(*datalen);
...
boolean Adafruit_FONA::HTTP_POST_start(char *url,
		FONAFlashStringPtr contenttype, const uint8_t *postdata,
		uint16_t postdatalen, uint16_t *status, uint16_t *datalen)
{
	if (!HTTP_setup(url))
		return false;
		...
		int32_t suffix2, uint16_t timeout)
{
	...
	DEBUG_PRINT(F("\t---> "));DEBUG_PRINT(prefix); DEBUG_PRINT(suffix1, DEC);DEBUG_PRINT(',');DEBUG_PRINTLN(suffix2, DEC);
		...
		FONAFlashStringPtr suffix, uint16_t timeout)
{
	...
	DEBUG_PRINT(F("\t---> "));DEBUG_PRINT(prefix); DEBUG_PRINT('"');DEBUG_PRINT(suffix);DEBUG_PRINTLN('"');
...
boolean Adafruit_FONA::sendCheckReply(FONAFlashStringPtr prefix,
		int32_t suffix1, int32_t suffix2, FONAFlashStringPtr reply,
		uint16_t timeout)
{
...
boolean Adafruit_FONA::sendCheckReplyQuoted(FONAFlashStringPtr prefix,
		FONAFlashStringPtr suffix, FONAFlashStringPtr reply, uint16_t timeout)
{
...
boolean Adafruit_FONA_3G::sendParseReply(FONAFlashStringPtr tosend,
		FONAFlashStringPtr toreply, float *f, char divider, uint8_t index)
{
	getReply(tosend);
